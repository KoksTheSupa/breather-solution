...
def plot(x: int, y: list) -> None:
    ...
    graph.annotate(
        f"Local max = {max(y)}",
        xy=(xmax, max(y)),
        xytext=(xmax, max(y) + Decimal(0.10)),
...
def plot3D(x: int, y: list) -> None:
    ...
    for xi, yi, zi in zip(z, x_list, y):
        if xi < 0:
            line = art3d.Line3D(
                *zip((xi, yi, 0), (xi, yi, zi)), marker=11, markevery=(1, 1)
            )
        else:
            line = art3d.Line3D(
                *zip((xi, yi, 0), (xi, yi, zi)), marker=10, markevery=(1, 1)
            )
        ax.add_line(line)
...
def counter(start: Decimal, N: int) -> list[Decimal]:
    ...
    S[1] = (
        (
            -a * SQDJ * Decimal(sqrt(1 - S[0] ** 2))
            + S[0]
            * Decimal(sqrt(1 + DJ**2 * Decimal(((1 - S[0] ** 2))) - Decimal(a**2)))
        )
        / Decimal((1 + DJ**2 * Decimal(1 - S[0] ** 2)))
    ).quantize(Decimal("1.000000000000000"), ROUND_HALF_UP)
    for i in range(1, N):
        A = (
            OMEGA * S[i]
            + (2 * B * S[i]) * Decimal(sqrt(1 - S[i] ** 2))
            + S[i - 1] * SQDJ * Decimal(sqrt(1 - S[i] ** 2))
            - S[i] * Decimal(sqrt(1 - S[i - 1] ** 2))
        )
        S[i + 1] = (
            (
                -A * SQDJ * Decimal(sqrt(1 - S[i] ** 2))
                + S[i]
                * Decimal(
                    sqrt(Decimal(1 - A**2) + DJ**2 * Decimal((1 - S[i] ** 2)))
                )
            )
            / Decimal((1 + DJ**2 * Decimal(1 - S[i] ** 2)))
        ).quantize(Decimal("1.000000000000000"), ROUND_HALF_UP)
        if isnan(S[i + 1]):
            break
    return S
...
def check(last: Decimal, prelast: Decimal) -> bool:
    ...
    temp = fabs(
        Decimal(1.0)
        - (
            last * Decimal(sqrt(1 - prelast**2))
            - (prelast + 2 * B * last) * Decimal(sqrt(1 - last**2))
        )
        / (last * OMEGA)
    )
    return temp < EPS
...
def energy(chain: list) -> Decimal | Literal[0]:
    ...
    for i in range(N):
        ...
        sum3 = Decimal(
            sqrt((1 - chain[i] * chain[i]) * (1 - chain[i + 1] * chain[i + 1]))
        )
        sum4 = Decimal(sqrt(1 - chain[i] * chain[i]))
        E += (
            -Decimal(sqrt(1 + DJ**2)) * sum1
            + Decimal("0.15") * sum2
            - sum3
            - Decimal("1.5") * sum4
            - E0
